...
export async function claimVerb(verb, limit, { target = process.cwd(), packageRoot = PACKAGE, ask, send } = {}) {
  ...
  if (claimed.length === 0) return { text: [`rigger ${verb}: from board ${project}, no card was pullable`, ...refused].join('\n'), code: 0 };
  ...
  return {
    text: [
      ...claimed.map((card) => `rigger ${verb}: claimed #${card.number} from board ${project}, and it was not worked: dispatch arrives with M2 and M4`),
      ...refused,
    ].join('\n'),
    code: 1,
  };
