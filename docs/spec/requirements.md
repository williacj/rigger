...
| R-CARD-15 | — a fenced code block opens at a line indented at most three spaces that opens with three or more backticks or three or more tildes; | the engine | the test suite | D2 |
| R-CARD-16 | — a fenced code block closes at the next line indented at most three spaces that holds only a run of at least as many of the same character; | the engine | the test suite | D2 |
...
| R-CARD-20 | — a heading's level is its count of `#`, and its text is the rest of the line, trimmed, stripped of any closing run of `#`, and trimmed again; | the engine | the test suite | D2 |
...
| R-CARD-25 | — a task-list item, whose text opens with `[ ]`, `[x]` or `[X]`, and a bullet whose text is empty or only whitespace, are never items. | the engine | the test suite | D2 |
