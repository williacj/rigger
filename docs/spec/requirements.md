...
| R-CARD-12 | Rigger reads a card's acceptance from the source text of the card's body, never from a rendering of it. The body splits into lines at each line feed, and at each carriage return directly followed by a line feed, and nowhere else. A line's indentation is the run of space characters at its start, and what the line opens with is what follows that run. A fenced code block opens at a line indented at most three spaces that opens with three or more backticks or three or more tildes. It closes at the next line indented at most three spaces that holds only a run of at least as many of the same character. A fenced code block never closed runs to the end of the body. Every line from its opening line to its closing line is inside it, and no line inside it is an ATX heading line or an item. An ATX heading line is any other line indented at most three spaces that opens with one to six `#`, then a space or the line's end. Its level is its count of `#`. Its text is the rest of the line, trimmed, stripped of any closing run of `#`, and trimmed again. The acceptance is every item in every section that opens at an ATX heading line whose text is exactly `Acceptance`. A section ends at the next ATX heading line of the same level or higher, meaning one with as many `#` or fewer. An item is a line in a section, indented at most three spaces, that opens with `-`, `*` or `+` and a space. Its text is the rest of the line after that marker and that one space. A task-list item, whose text opens with `[ ]`, `[x]` or `[X]`, is never an item. A bullet whose text is empty or only whitespace is never an item either. | the engine | the test suite | D2 |
