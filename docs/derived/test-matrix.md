...
| R-SAFE-3 | **gap** |
| R-SAFE-4 | **gap** |
| R-SAFE-5 | `test/doctor.test.mjs` doctor refuses the source tree it is running from, and asks nothing before it does<br>`test/doctor.test.mjs` a worktree beside the source tree is not the source tree, however its name begins<br>`test/doctor.test.mjs` a second spelling of the source tree is still the source tree<br>`test/doctor.test.mjs` the tree compared is the repository, not the directory the command was run in |
