...
| R-SAFE-3 | **gap** |
| R-SAFE-4 | **gap** |
| R-SAFE-5 | `test/doctor.test.mjs` doctor refuses the source tree it is running from, and asks nothing before it does<br>`test/doctor.test.mjs` a worktree beside the source tree is not the source tree, however its name begins<br>`test/doctor.test.mjs` a second spelling of the source tree is still the source tree<br>`test/doctor.test.mjs` the tree compared is the repository, not the directory the command was run in<br>`test/doctor.test.mjs` a tree git cannot name is refused rather than compared against the working directory<br>`test/once.test.mjs` once run against the source tree it is running from exits non-zero, names R-SAFE-5, and claims nothing<br>`test/plan.test.mjs` plan run against the source tree it is running from exits non-zero, names R-SAFE-5, and reads no board<br>`test/report.test.mjs` report run against the source tree it is running from exits non-zero and names R-SAFE-5<br>`test/run.test.mjs` run run against the source tree it is running from exits non-zero, names R-SAFE-5, and claims nothing<br>`test/setup-board.test.mjs` setup-board run against the source tree it is running from exits non-zero, names R-SAFE-5, and writes nothing |
